...
def autonomous_executor(state: dict):
    ...
    with open("C:\\Langgraph Agent\\my_agent\\utils\\query_rules.txt", "r") as f:
        query_guidelines = f.read()

    

    langchain_tools = state.get("langchain_tools", [])
    llm_with_tools = slightly_creative_llm.bind_tools(langchain_tools)

    while True:
        # Build the prompt with all context
        prompt = f"""You are a Tableau data assistant with full access Tableau Server via MCP tools,
        you have the user's input, an implementation plan, context anchors (Key entities) and the history of executed tools.

        Your workflow:

        1. Understand the user's request and implementation plan:
        - Review the user input to fully comprehend the user's goal.
        - Consider context from previous tool executions in tool_execution_history.

        2. Plan and execute tool calls automatically:
        - Use MCP tools as needed based on the implementation plan.
        - For each step in the implementation plan, determine which tool to call and with what arguments.
        - Field names and key entities like metrics , filters, dimensions used in user request, implementation plan and 
          context anchors are business specific and they should be mapped correctly with the field names in the datasource metadata or
          structure of tableau server when making tool calls.
        - Also review the tools meta data to understand the correct syntax for filters and aggregations when making tool calls.
        - Execute tool calls in proper order without asking for confirmation.
        - After each tool call, review the results and determine the next steps based on the implementation plan and the results obtained so far.
        - Continue chaining tool calls until the user's request is fulfilled or no further tool calls can be made.

        3. Process tool results and generate final response:
        - Summarize results in a concise, human-readable answer.
        - Briefly explain what you did, like "Queried sales data for branch 0949 and found total sales of $X", or "Filtered employee data for department Y and calculated average tenure of Z years".
        - Include explanations for any adjustments made during retries (e.g., fuzzy matching or corrected filter values).
        - If a query could not be completed, explain why in plain English, including what was attempted.

        Tool usage guidelines:
        - Use listing tools to explore available datasources and their metadata before making queries. always call these without any parameters
        - Filter syntax: `"field:operator:value"` (operators: eq, in, gt, gte, lt, lte)
        - Use exact field names from datasource metadata.
        - Chain tool calls as needed.
        - Standard workflow:
        - List datasources → get datasource ID
        - Get datasource metadata → see available fields
        - Query datasource → use correct field names and aggregations

        
        OPEN ENDED SEARCH CONSTRAINTS:
        If user request contains any string value which could not be matched directly. For example, user asked for data of branch name: "Clifton Branch" 
        but in the datasource metadata the branch name is "Clifton Karachi Branch" or "Clifton Branch (Karachi)" then you should not return empty results.
        Instead you need to handle this situation very cleverly, for example user has also provided some other contraints in the input message like date range or 
        city or region or some other filter values which can be used to narrow down the search and then you can apply those deterministic feilds and retrive all branch names,
         then filter which names contain the word "Clifton" and then you can apply fuzzy matching on those remaining names to find the closest match for "Clifton Branch" and use that in your tool call.
         
         
        IMPORTANT RESTRICTIONS:
        - If a limit value is provided, use that. For example, if the user says "Get top 5 products by sales", you can use a limit of 5 in your tool call.
        - If a limit value is not provided in the user request, use a default of 10 rows for any query that could return large results.
        - Never try to run tool with unsupported parameters or operators.
        - NEVER use unsupported keys.

        ERROR HANDLING & FUZZY MATCHING:

        If a filter value is invalid:
        - Parse error for suggestions, pick closest match (case-insensitive, partial match).
        - Retry query with corrected value.
        - Mention in final response: "Note: Exact match for 'X' not found. Showing results for 'Y' instead, which is the closest match."
        ...
        {query_guidelines}
"""
        # # print("tool_execution_history:", state.get("tool_execution_history", []))
        # Invoke LLM
...
async def query_datasource_tool(state: dict):
    ...
    # Try parsing JSON, try to fix minor issues
    if isinstance(query, str):
        try:
            query = json.loads(query)
        except json.JSONDecodeError:
            # Attempt minor fixes (e.g., replace single quotes with double quotes)
            fixed_query = query.replace("'", '"')
            try:
                query = json.loads(fixed_query)
            except json.JSONDecodeError:
                print("Failed to parse query even after fixes. Using original string.")
    ...
    if "validation" in response and "No values found" in response:
        # print("error detected")
        # # print("Filtered metadtaa : ", filtered_meta_data)
        ...
        #         query = tool_args.get("query", {}) 
        #         query_fileds = query.get("Fields")

        #         for f in query_fileds:
        #             query_field_name = f.get("fieldCaption", "")

        return {"string_validation" : "fail",
                "tool_call_under_str_resolution": new_tool_call,
                "filtered_metadata": filtered_meta_data}
...
def evaluator_node(state: State):
    ...
    msg = llm.invoke(f"""
            ...
            2. Which can be a follow-up question (is_follow: {is_follow}) with enhanced input:
            {enhanced_input}
