...
from .llm import llm, slightly_creative_llm, creative_llm, reasoning_llm, reasoning_expert_llm
import json
import re
import uuid
...
def other_domain_message_handler(state: State):
    ...
    return {"output": text, "conversation_history": [AIMessage(content=text)]}
...
def manage_conversation_history(state: State):
    ...
    if len(conversation_history) > 21:
        ...
        if isinstance(msg.content, list):
            ...
            text = msg.content        
        conversation_history = [SystemMessage(content=f"Summary of previous conversation: {text}")] + conversation_history
...
def enhance_new_prompt(state: State):
    ...
    msg = llm.invoke(f"""You are a Tableau data analytics assistant with access to Tableau MCP tools.
    ...
    if isinstance(msg.content, list):
        ...
        text = msg.content  
    print("Enhanced new prompt:", text)  
    return {"enhanced_input": text,
...
def enhance_follow_up_prompt(state: State):
        ...
        print("Enhanced follow-up query:", text)
        return {
            "enhanced_input": text,
            "conversation_history": [HumanMessage(content=text)]
        }
...
def plan_implementation(state: State):
    ...
    if state.get("need_tools") == "True":
        ...
        if isinstance(msg.content, list):
            ...
            implementation_plan = msg.content
...
def autonomous_executor(state: dict):
    ...
    llm_with_tools = reasoning_expert_llm.bind_tools(langchain_tools)
    tool_execution_history = state.get("tool_execution_history", [])
    ...
    while True:
        ...
        - Always filter string fields with Filtertype 'MATCH' and filter 'contains'
...
def summarize_string_validation_process(previous_summary,tool_execution_history):
    prompt = f"""Your task is to create rolling summary. You are given which a list of tool calls and their results, which are comming from a multi turn tool execution process resposible to search for possible match of the substring based on actual value. 
    
    For example, if original query containing filter value = 'Jhehlum' , but actual name in datasource is 'Jhelum', then the agent splits actual value into substring candidates like 'jhe', 'hlem' and 'lum' and excutes queries with these substrings. 
    
    Your task is to summarize this raw tool calls and results into strutured JSON.
    You will look for the filters using contains key word and a value, for example:
            "filterType": "MATCH",
            "contains": "Clifton Karachi",
            "exclude": false

    For each such field, you will summarize it like:
    Fieldname, cadidate tried, results of that cadidate.
    ...
    - in case of tool call result being error, keep the summary of error only.
    - don't add any addtional explanations or reasoning

    Tool usage guidelines (strict):
    - Do not use any limit.
    - Never use unsupported keys or unsupported operators.
    - Tool Call must be a Valid JSON object, No string or Stringified JSON
    - Specifically, check the query filed to be JSON,  when calling query datasource tool
            """
...
async def string_validation_node(tool_call, user_msg, meta_data, tools):
    ...
    while flag and attempts < 4:
        ...
        Tableau MCP Server. You have access to use query-datasource tool and you are provided with tool call arguments, metadata of the fileds
        and progress history to see how many matches you have found. 
        
        The issue in query is failure in string filtering. As a solution, your most relaible way to filter results on string type columns is by using MATCH Filter with 'contains' keyword. This can match even the substrings, but problem accurs when there is a small mismatch 
        ...
        1. If original query itself contains one word to be filtered, try that first. If it fails, split the word into subwords, but not more   than 3 parts.
        2. If query returns no results and error message suggests some closet candidates, then use them.
        3. Never repeat any cadidate again.
        ...
        to output only the STRING filed as your main goal is to check if this exact value does not exist in the string column then
        what is the most closet match. 
        After running query with all options, return the results.

        Original Query:{tool_call}
        Meta Data of fields: {meta_data}
        The tool execution history below, tells you which candidates you have tried so far, and what was their results:{summarized_histrory}
        ...
        STOP CONDITIONS (MANDATORY):
            1. If no new unique candidate substrings remain → STOP.
            2. If all words from original string have been tried → STOP.
            3. If same tool call arguments are about to be repeated → STOP.
            4. If at least one valid match found → Return final result.


        Your output must be an AIMessage type with tool calls or the final human readable result after completing your task.
        """
        print(prompt)
        msg = llm_with_tools.invoke(prompt)
...
async def query_datasource_tool(state: dict):
    ...
    if "Filter validation failed for field" in response:
        ...
        tool_calls_history, final_msg = await string_validation_node(tool_call, user_input, filtered_meta_data, langchain_tools)
...
def execution_logs_node(state: dict):
    ...
    if isinstance(msg.content, list):
        ...
        text = msg.content
    ai_message = AIMessage(content="The plan execution logs so far: " + text)
...
async def list_datasources_tool(state: dict):
    ...
    tool_call = {'name': 'list-datasources', 'args': {'filter': '', 'pageSize': 100, 'limit': 100}, 'id': 'call_2vUAGEKOvb3WLESS14A1PL6k', 'type': 'tool_call'}
